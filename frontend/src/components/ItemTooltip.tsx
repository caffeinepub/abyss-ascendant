...
function AffixLine({ affix }: { affix: ItemAffix }) {
  ...
  if (!display) {
    return (
      <div className="flex items-center gap-2 text-xs text-muted-foreground">
        <span>•</span>
        <span>+{affix.value} {affix.label}</span>
      </div>
    );
  }
  ...
  return (
    <div className={`flex items-center gap-2 text-xs ${display.color}`}>
      ...
      <span>
        +{affix.value}{suffix} {display.label}
      </span>
    </div>
  );
}
    ...
    >
      <div className="flex items-center gap-2">
        ...
        <div className="min-w-0 flex-1">
          <div className={`font-semibold text-xs truncate ${rarityClass.split(' ')[0]}`}>
            {item.name}
          </div>
      ...
      {item.affixes.length > 0 && (
        <div className="mt-1 space-y-0.5">
          {item.affixes.map((affix, i) => {
            ...
            const suffix = affix.stat === 'critChance' ? '%' : '';
            return (
              <div key={i} className={`text-xs ${display?.color ?? 'text-muted-foreground'}`}>
                +{affix.value}{suffix} {display?.label ?? affix.label}
              </div>
            );
          })}
        </div>
