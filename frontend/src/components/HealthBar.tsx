...
export default function HealthBar({ currentHP, maxHP, className = '', compact = false }: HealthBarProps) {
  ...
  if (compact) {
    return (
      <div className={`flex items-center gap-1.5 ${className}`}>
        <Heart className="w-3.5 h-3.5 text-health-low flex-shrink-0" fill="currentColor" />
  ...
  return (
    <div className={`flex items-center gap-2 ${className}`}>
      <Heart className="w-4 h-4 text-health-low flex-shrink-0" fill="currentColor" />
