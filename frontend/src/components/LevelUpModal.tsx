...
export default function LevelUpModal({
  ...
  const totalAllocated = allocated.str + allocated.dex + allocated.int + allocated.vit;
      ...
      <div className="bg-surface-1 border border-border rounded-lg w-full max-w-md shadow-2xl">
        ...
        <div className="p-5 border-b border-border text-center">
          <div className="flex items-center justify-center gap-2 mb-1">
            ...
            <h2 className="text-2xl font-cinzel font-bold text-accent">Level Up!</h2>
          </div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            You have{' '}
            <span className="text-accent font-bold">{statPointsToSpend}</span> stat point
        ...
        <div className="px-5 pb-2 text-center">
          <span className="text-sm text-muted-foreground">
            Remaining points:{' '}
            <span className={remaining > 0 ? 'text-accent font-bold' : 'text-foreground font-bold'}>
              {remaining}
            </span>
          </span>
        </div>
        ...
        <div className="p-5 pt-3 flex gap-3">
          ...
          >
            {remaining > 0 ? `Spend ${remaining} more point${remaining !== 1 ? 's' : ''}` : 'Confirm'}
          </button>
        </div>
      </div>
    </div>
