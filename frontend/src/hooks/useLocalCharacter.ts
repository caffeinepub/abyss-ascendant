...
function resolveCharacterClass(backendClass: string): CharacterClass {
  if (backendClass === 'Warrior' || backendClass === 'Rogue' || backendClass === 'Mage') {
    return backendClass;
  }
  return 'Warrior';
}
...
function buildLocalCharacter(
  ...
  const baseStats: BaseStats = { str: baseStr, dex: baseDex, int: baseInt, vit: baseVit };
  ...
  const rawHp = currentHpOverride !== undefined ? currentHpOverride : backendCurrentHp;
  ...
  if (equippedAbilities.length === 0 && backendChar.equippedAbilities?.length > 0) {
...
export function useLocalCharacter(
  ...
  }, [characterId, backendChar]); // eslint-disable-line react-hooks/exhaustive-deps
    ...
    (newHp: number) => {
      ...
      setLocalCharacter((prev) => {
        ...
        saveStoredData(characterId, {
          equippedAbilities: stored?.equippedAbilities || prev.equippedAbilities,
  ...
  const updateBaseStats = useCallback(
    (newBaseStats: BaseStats, newTotalSpent: number) => {
      ...
      setLocalCharacter((prev) => {
        ...
        const unspentStatPoints = calculateUnspentStatPoints(prev.totalStatPointsEarned, newTotalSpent);
  ...
  const updateEquipment = useCallback(
    (equippedItems: GeneratedItem[], inventory: GeneratedItem[], stash: GeneratedItem[]) => {
      if (characterId === null) return;
      setLocalCharacter((prev) => {
        if (!prev) return prev;
        const stored = loadStoredData(characterId);
