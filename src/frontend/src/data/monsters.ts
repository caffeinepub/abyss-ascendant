...
export function generateMonster(): GeneratedMonster {
  ...
  const baseHp = Math.max(
    10,
    template.baseHp + prefix.modifiers.hp + suffix.modifiers.hp,
  );
  const baseAttack = Math.max(
    1,
    template.baseAttack + prefix.modifiers.attack + suffix.modifiers.attack,
  );
  const baseDefense = Math.max(
    0,
    template.baseDefense + prefix.modifiers.defense + suffix.modifiers.defense,
  );
