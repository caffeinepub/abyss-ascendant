...
/**
 * Compute the total stat bonuses contributed by all equipped items.
 * This is the single source of truth for equipment stat aggregation.
 */
export function computeEquipmentBonuses(
  ...
  for (const item of equippedItems) {
    for (const affix of item.affixes) {
      switch (affix.stat) {
        case "str":
          result.str += affix.value;
          break;
        case "dex":
          result.dex += affix.value;
          break;
        case "int":
          result.int += affix.value;
          break;
        case "vit":
          result.vit += affix.value;
          break;
        case "hp":
          result.bonusHp += affix.value;
          break;
        case "physicalDamage":
          result.bonusPhysicalDamage += affix.value;
          break;
        case "magicDamage":
          result.bonusMagicDamage += affix.value;
          break;
        case "defense":
          result.bonusDefense += affix.value;
          break;
        case "critChance":
          result.bonusCritChance += affix.value;
          break;
      }
    }
    // Base weapon damage and base armor defense
    if (item.itemType === "Weapon")
      result.bonusPhysicalDamage += item.baseDamage ?? 0;
    if (item.itemType === "Armor") result.bonusDefense += item.baseDefense ?? 0;
  }
  return result;
}
